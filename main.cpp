...
void gameInit()//游戏初始化
{
	...
	for (int i = 0; i < 10; i++)
	{
		sprintf_s(name, "res/progress/progress%d.png", i);
		loadimage(&imgjindu[i], name);
	}
	//初始化进度条
	loadimage(&imgjindutiao,"res/progress/progress.png");
	//初始化僵尸头
	loadimage(&imgzmhead, "res/progress/head.png");
}
//---------------------------------------------------------------------------------------
	...
	if (peekmessage(&msg))//判断当前有没有消息
	{
		if (msg.message == WM_LBUTTONDOWN)//鼠标左键按下
		{
			if (msg.x > 338 && msg.x < 338 + 65 * ZHI_WU_COUNT && msg.y < 96)//判断鼠标是否点击植物卡片
			{
				...
				{
					mciSendString("play res/seed_up.wav", 0, 0, 0);
				}
				...
				{
					mciSendString("play res/sun_lack.wav", 0, 0, 0);
				}
				status = 1;//鼠标拖动
		...
		else if (msg.message == WM_LBUTTONUP && status == 1)//鼠标左键抬起
		{
			if (msg.x > 256 - 112 && msg.y > 179 && msg.y < 489)//种植范围(只能种植草坪)
			{
				...
				if (map[row][col].type == 0)//判断这个地方没有种植植物
				{
					...
					{
						sunshine -= 100;
						mciSendString("play res/plant.wav", 0, 0, 0);
	...
	if (count > 2 * 2)
	{
		...
		for (int i = 0; i < zmMax; i++)
		{
			if (zms[i].used)//在使用
			{
				...
				{
					mciSendString("play res/alarm.mp3", 0, 0, 0);
				}
				if (zms[i].x < 48)//到达房子
				{
					gameStatus = FAIL;
			...
			if (zms[i].used)//在使用
			{
				if (zms[i].dead)//僵尸死亡
				{
					...
					if (zms[i].frameIndex >= 20)
					{
						...
						mciSendString("play res/zm_die.mp3", 0, 0, 0);
						killCount++;
						if (killCount == ZM_MAX)
						{
							gameStatus = WIN;
				...
				else if (zms[i].eating)
				{
					...
					mciSendString("play res/eat.wav", 0, 0, 0);
				}
				else
				{
					zms[i].frameIndex = (zms[i].frameIndex + 1) % 21;//帧动作
	...
	for (int i = 0; i < bCount; i++)
	{
		...
		for (int k = 0; k < zCount; k++)
		{
			if (zms[k].used == false)continue;//僵尸没有出来
			{
				...
				{
					mciSendString("play res/pea_hit.wav", 0, 0, 0);
					zms[k].blood -= 10;//僵尸扣血
...
void startUI()//启动菜单
{
	...
	//木栏下滑
	mciSendString("play res/wood.mp3", 0, 0, 0);
	...
	while (1)
	{
		...
		if (peekmessage(&msg))//判断是否有消息
		{
			...
			{
				mciSendString("play res/zm_laugh.mp3", 0, 0, 0);
				BeginBatchDraw();
				putimage(0, 0, &imgBg);//打印背景
				putimagePNG(60, 0, &imgWoodSign1);//木栏1
				putimagePNG(60, 115, &imgWoodSign2);//木栏2
...
void viewScence()
{
	...
	mciSendString("play res/transition.wav", 0, 0, 0);//循环播放
	...
	mciSendString("stop res/transition.wav", 0, 0, 0);//停止播放
}
//------------------------------------------------------
...
void gamestart()
{
		mciSendString("play res/start_plant.wav", 0, 0, 0);
